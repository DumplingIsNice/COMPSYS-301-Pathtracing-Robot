...
void GenerateDirectionQueue()
{
	...
	while (IsElementValid(next->tail)) {
		...
		if (GetNodeDataAdjacentPaths(next->node) > 1) { AddDirection(prev->node, current->node, next->node); } // only find for intersection nodes (where there must be more than 2 prev/next nodes)
	}
